...
scanFolder('./source',fs);

function scanFolder(path,fs){
  fs.readdir(path, function(err, files) {
    if (err) return console.log("读取目录错误：",err);
    files.forEach(function(file) {
      var curPath = path + "/" + file;
      fs.stat(curPath, function(err, stats) {
        if (err) return console.log("读取文件信息错误：",err);
        if(stats.isDirectory()){
          scanFolder(curPath,fs);
        }else{
          if(curPath.indexOf(".md")>0){
            writeFileTime(curPath,fs);
          }
        }
      });
    });
